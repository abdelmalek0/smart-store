...
class ModelRuntime {
  ...
  ModelRuntime(this.modelPath);

  /// Initialize the worker for this model
  Future<void> init() async {
    if (_isInitialized) return;
    if (_isDisposed) {
      throw Exception("Cannot re-initialize disposed ModelRuntime");
    }

    debugPrint(
      "ModelRuntime: Spawning worker for ${modelPath.split('/').last}",
    );
    ...
    receivePort.listen((message) {
      if (message is SendPort) {
        ...
        debugPrint(
          "ModelRuntime: Received SendPort for ${modelPath.split('/').last}",
        );
      } else if (message is WorkerReady) {
        if (message.success) {
          debugPrint(
            "ModelRuntime: Worker ready for ${modelPath.split('/').last}",
          );
          if (!completer.isCompleted) completer.complete();
        } else {
          debugPrint(
            "ModelRuntime: Worker failed for ${modelPath.split('/').last}: ${message.error}",
          );
        ...
        if (message.error != null) {
          debugPrint(
            "⚠️ ModelRuntime: Error for ${message.streamId}: ${message.error}",
          );
        }
    ...
    try {
      ...
      _isInitialized = true;
      debugPrint(
        "ModelRuntime: Worker for ${modelPath.split('/').last} fully initialized",
      );
    } catch (e) {
      debugPrint(
        "ModelRuntime: Worker initialization failed/timed out for $modelPath: $e",
      );
      dispose();
      rethrow;
    }
  ...
  void enqueueFrame(
    ...
    if (!_isInitialized || _workerSendPort == null) {
      debugPrint("⚠️ ModelRuntime not initialized for $modelPath");
      return;
    }
  ...
  Future<void> dispose() async {
    ...
    debugPrint(
      "ModelRuntime: Disposing worker for ${modelPath.split('/').last}...",
    );
